...
def nationality_search(countries, cur, conn):
    playersList = []
    info = cur.execute("SELECT name, position_id, nationality from Players")
    cs = info.fetchall()
    for x in cs:
        if x[2] in countries:
...
def birthyear_nationality_search(age, country, cur, conn):
    playersList = []
    info = cur.execute("SELECT name, nationality, birthyear from Players")
    cs = info.fetchall()
    before = 2023 - age
    for x in cs:
        if x[1] == country and x[2] < before:
            playersList.append((x))
    return playersList
...
def position_birth_search(position, age, cur, conn):
    playersList = []
    info = cur.execute("SELECT Players.name, Positions.position, Players.birthyear FROM Players JOIN Positions on Players.position_id = Positions.id")
    cs = info.fetchall()
    after = 2023 - age
    for x in cs:
        if x[1] == position and x[2] > after:
            playersList.append((x))
    return playersList
